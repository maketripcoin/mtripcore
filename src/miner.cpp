...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MaketripMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MaketripMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MaketripMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("maketrip-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MaketripMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
