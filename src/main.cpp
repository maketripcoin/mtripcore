...
#if defined(NDEBUG)
#error "Maketrip cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 2500) {
        nSubsidy = 0.1 * COIN;
    } else if (nHeight > 2500 && nHeight <= 7500) {
        nSubsidy = 30 * COIN;
...
{
    RenameThread("maketrip-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MTRIP: We use certain sporks during IBD, so check to see if they are
